...
## Method
The solution was implemented using Ansible on a management virtual machine to automate the deployment of a Prometheus and Grafana-based monitoring stack across five Rocky Linux virtual machines running Podman. Node Exporter was deployed on all hosts to collect hardware-level metrics, and Prometheus Podman Exporter was selected over cAdvisor for container-level metrics, as cAdvisor proved incompatible with Podman's daemonless architecture — producing inconsistent container identification and missing network and filesystem data. Prometheus was deployed on the dedicated metrics-01 VM with static scrape targets configured for all hosts, and Grafana was co-deployed on the same machine and provisioned automatically through configuration files covering datasources, dashboards, and alerting rules, eliminating the need for manual setup through the web interface. An alert rule was defined to trigger when root filesystem usage on any VM exceeded 90%, with notifications delivered via a Postfix mail relay to internal addresses. Grafana was further integrated with FreeIPA through LDAP over TLS, mapping the sysadmins group to the Admin role and all remaining authenticated users to the Viewer role.
